...
/*
  ...
  Behavior:
    ...
    - no Serial code is used, keeping the compiled sketch smaller.
  ...
  Important:
    ...
    - changing the configuration requires recompiling the sketch.

  Compare build sizes:
    1. compile this example and note Flash and SRAM usage in the Arduino IDE
       build report;
    2. compile an example using the default configuration;
...
/*
  ...
  scheduler queue. This sketch uses only one PERIODIC task.
*/
...
/*
  All compile-time configuration macros must appear before this include.
*/
